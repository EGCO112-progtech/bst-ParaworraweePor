...
void insertNode( BST* b, int value ){
   ...
   if(!b->root) b->root=new_node;
         ...
         while(!inserted){
            if(t->data >= value){
               if(t->leftPtr==NULL){ //ว่างแล้วแทรกได้เลย
               t->leftPtr=new_node;
               inserted++;
               } 
               else t=t->leftPtr; //ถ้าไม่ว่างก้เลื่อนไปก่อนแล้วแทรกตัวที่ว้าง
            }
            else{
               if(t->rightPtr==NULL){ //ว่างแล้วแทรกได้เลย
               t->rightPtr=new_node;
               inserted++;
               } 
               else t=t->rightPtr; //ถ้าไม่ว่างก้เลื่อนไปก่อนแล้วแทรกตัวที่ว้าง
            }
         }
      }
...
void printbst( TreeNodePtr treePtr, int count )
{
   int i;
   if( treePtr != NULL )
   {  
